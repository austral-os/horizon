...
NewDownloadDialog::NewDownloadDialog(std::function<void(std::string)> on_accept)
    ...
    textbox->set_text("https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb");
    ...
    auto download_btn = std::make_unique<horizon::Button<horizon::SolidObject>>();
    download_btn->set_text("Descargar");
    download_btn->set_accent_color(WidgetAccentColor::Primary);
    download_btn->set_size(120, 35);
    download_btn->when_click.connect([this, textbox_ptr](horizon::MouseButtonEventContext&) {
