...
public:
    InstallerWindow(bool oobe_mode, const std::string &initial_view = "", bool working_mode = false)
        : Window("Horizon Installer"), m_oobe_mode(oobe_mode), m_initial_view(initial_view),
...
private:
    void setup_language_page()
    {
        ...
        lang_page->when_continue.connect(
            [this, ptr](auto &)
            {
    ...
    void setup_remaining_pages()
    {
        if (m_oobe_mode)
        {
            ...
            user_page->when_continue.connect(
                [this, user_ptr](auto &)
                {
        ...
        else
        {
            ...
            license_page->when_disagree.connect(
                [this](auto &)
                {
            ...
            region_page->when_continue.connect(
                [this, region_ptr](auto &)
                {
            ...
            disk_page->when_install.connect(
                [this, disk_page_ptr](auto &)
                {
        ...
        install_page->when_cancel.connect(
            [this](auto &)
            {
                // Handle cancellation
            });
        ...
        success_page->when_finish.connect(
            [this](auto &)
            {
    ...
    void start_installation()
    {
        ...
        if (m_working_mode)
        {
            ...
            LOG_INFO << "[DEV] Target: " << m_config.target_device
                     << ", OOBE: " << (m_oobe_mode ? "YES" : "NO");
            ...
            std::thread(
                [this]()
                {
                    for (int i = 0; i <= 100; i += 10)
                    {
                        ...
                        if (auto *app = application())
                        {
                            app->post_task(
                                ...
                                {
                                    if (m_install_page_ptr)
                                        m_install_page_ptr->update_progress(
                                            i / 100.0f, "Simulating installation... " +
                                                            std::to_string(i) + "%");
                                });
                        }
                    }
                    if (auto *app = application())
                    {
                        app->post_task([this]() { m_wizard->next(); });
                    }
                })
                .detach();
            return;
        ...
        std::thread(
            ...
            {
                StepResult res =
                    m_oobe_mode ? m_manager->run_stage2(m_config) : m_manager->run_stage1(m_config);
    ...
    void jump_to_view(const std::string &view_name)
    {
        ...
        if (m_oobe_mode)
        {
            if (view_name == "new-user")
                index = 1;
            else if (view_name == "progress")
                index = 2;
            else if (view_name == "success")
                index = 3;
        }
        else
        {
            if (view_name == "welcome")
                index = 1;
            else if (view_name == "license")
                index = 2;
            else if (view_name == "region")
                index = 3;
            else if (view_name == "disk")
                index = 4;
            else if (view_name == "progress")
                index = 5;
            else if (view_name == "success")
