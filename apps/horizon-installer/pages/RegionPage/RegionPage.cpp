...
namespace horizon::installer
{
    RegionPage::RegionPage()
    {
        ...
        country_combo->set_fixed_size(35);
        ...
        if (!std::filesystem::exists(countries_path))
        {
            ...
            if (count != -1)
            {
                ...
                // Try different relative paths for development
                std::vector<std::string> dev_paths = {
                    dir + "/../apps/horizon_session/data/countries.json",
                    dir + "/../../../apps/horizon_session/data/countries.json"};
                for (const auto &dp : dev_paths)
                {
        ...
        if (std::filesystem::exists(countries_path))
        {
            try
            {
                ...
                {
                    for (auto &item : data["countries"])
                    {
                        std::string id = item.value("id", "");
                        std::string name = item.value("name", "");
                        if (!id.empty())
                        {
                            country_ptr->add_item(id, name);
                        }
                    }
                }
            }
        ...
        continue_btn->when_click.connect(
            [this](auto &)
            {
                EventContext ctx;
