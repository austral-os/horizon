...
namespace horizon::arkfm
{
    GoToFolderDialog::GoToFolderDialog()
        : WaylandWindow("horizon.arkfm.go_to_folder", 450, 210, false, false)
    {
        set_name(i18n().tr("arkfm.dialog.go_to_folder"));
        setup_ui();
    }

    void GoToFolderDialog::setup_ui()
    {
        auto window_widget = std::make_unique<horizon::Window>(i18n().tr("arkfm.dialog.go_to_folder"));
        ...
        auto prompt_label = std::make_unique<horizon::Label>(i18n().tr("arkfm.dialog.enter_path"));
        ...
        button_container->add_child(horizon::Spacer());
        ...
        accept_btn->when_click.connect(
            [this, text_box_ptr](auto &)
            {
                GoToFolderEvent ev;
                ev.sender = this;
                ev.path = text_box_ptr->text();
                if (ev.path.empty()) ev.path = "/";
                when_accepted.run(ev);
                this->on_close();
            });
        button_container->add_child(std::move(accept_btn));

        root_panel->add_child(horizon::Spacer());
