...
namespace horizon::arkfm
{
    MountPasswordDialog::MountPasswordDialog(const std::string& server_name) 
        : WaylandWindow("arkfm.dialog", 520, 360)
    {
        ...
        connect_as_row->add_child(std::make_unique<Label>("Conectarse como:"));
        ...
        connect->set_height(32);
        connect->when_click.connect([this](auto&) {
        ...
        m_guest_radio->when_mouse_press.connect([this](auto&) { 
            m_user_radio->set_selected(false);
            update_enabled_state(); 
        });
        m_user_radio->when_mouse_press.connect([this](auto&) { 
    ...
    {
        if (!m_user_radio || !m_name_input || !m_pass_input || !m_remember_check) return;
