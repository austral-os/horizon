...
namespace horizon
{
    LabwcAppAdapter::LabwcAppAdapter(Application *app)
    {
        setup_ipc();
        if (app)
        {
    ...
    std::vector<ApplicationInfo> LabwcAppAdapter::get_running_applications()
    {
        ...
        // Add foreign apps first (they are more "real" from compositor perspective)
        for (const auto &app : m_foreign_apps)
        {
            merged[app.app_id] = app;
        }

        // Add/Override with IPC apps (they might have more metadata like icons/dock status)
    ...
    void LabwcAppAdapter::setup_ipc()
    {
    ...
    void LabwcAppAdapter::merge_and_notify()
    {
    ...
    void LabwcAppAdapter::handle_ipc_message(const std::string &msg)
    {
        ...
        {
            LOG_ERROR << "LabwcAppAdapter: Error parsing IPC message: " << e.what();
        }
    }
