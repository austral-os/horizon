...
namespace horizon
{
    ...
    void AquaObject::draw(GraphicsContext &gc)
    {
        ...
        gc.drawRect(m_start_draw_x, m_start_draw_y - 1, m_width, m_height, m_corner_radius, 1.0f);

        // Outer border (gray/black)
        gc.setColor(border_color);
        gc.drawRect(m_start_draw_x, m_start_draw_y, m_width, m_height - 3, m_corner_radius, 1.5f);
        ...
        int h_margin_x = h_radius_val / 4; // Margen a los lados para que el brillo sea más chico
