...
namespace horizon::preferences
{
    ...
    WifiConnectDialog::WifiConnectDialog(const std::string& ssid, const std::string& ap_path, const std::vector<WifiDevice>& devices)
        : WaylandWindow("horizon.wifi_connect", 450, 300, true, false), m_ssid(ssid), m_ap_path(ap_path), m_devices(devices)
    {
    ...
    void WifiConnectDialog::setup_ui(const std::string& ssid)
    {
        ...
        btn_accept->when_click.connect([this](MouseButtonEventContext&) {
            ...
            const ComboItem* selected = m_device_combo ? m_device_combo->selected_item() : nullptr;
            if (!selected) return;
            ...
            if (m_device_combo) m_device_combo->set_enabled(false);
...
#include <thread>
    ...
    void WifiConnectDialog::perform_connection_async(const std::string& password, const std::string& device_path)
    {
        if (!m_dbus || !m_dbus->get_connection()) return;
        ...
        DBusMessage* reply = dbus_connection_send_with_reply_and_block(m_dbus->get_connection(), msg, -1, &dbus_err);
        ...
        if (dbus_error_is_set(&dbus_err) || reply == nullptr) {
            ...
            if (m_device_combo) m_device_combo->set_enabled(true);
        ...
        bool finished = false;
        int attempts = 0;
        while (!finished && attempts < 40) { // Max 20 segundos
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            attempts++;

            auto state_var = m_dbus->get_property("org.freedesktop.NetworkManager", active_conn_path, 
