...
namespace horizon::preferences
{
    ...
    void DesktopView::create_section_ac(horizon::Widget* parent)
    {
        ...
        m_images_view->when_image_selected.connect([this](const std::string& path) {
    ...
    void DesktopView::create_section_d(horizon::Widget* parent)
    {
        ...
        auto add_btn = std::make_unique<horizon::Button<horizon::AquaObject>>();
        add_btn->set_text("+");
        add_btn->set_size(30, 30);
        m_add_button = add_btn.get();
        buttons_hbox->add_child(std::move(add_btn));

        auto rem_btn = std::make_unique<horizon::Button<horizon::AquaObject>>();
        rem_btn->set_text("-");
        rem_btn->set_size(30, 30);
        m_remove_button = rem_btn.get();
        buttons_hbox->add_child(std::move(rem_btn));

        parent->add_child(std::move(buttons_hbox));
    ...
    void DesktopView::update_tree_view()
    {
        ...
        for (const auto& source : m_sources)
        {
            ...
            {
                auto route_item = std::make_unique<horizon::TreeViewItem>("folder-open", route.name);
                ...
                if (!m_first_route_to_select) m_first_route_to_select = route_item_ptr;
