...
std::unique_ptr<Menu> TopPanelMenuBar::create_system_menu()
{
    ...
    auto add_sys_item = [&](const std::string& text, const std::string& id = "") {
        ...
        item->when_click.connect([this, id, text](auto&) {
            ...
            if (item_id == "run_terminal") m_app->send_remote_signal(-1, "run_app", "konsole");
            else if (item_id == "run_aboutus") ApplicationLauncher::launch("about-us");
            else if (item_id == "run_logout") m_app->send_remote_signal(-1, "logout");
            else if (item_id == "force_quit" && m_current_owner_pid != -1) m_app->send_remote_signal(m_current_owner_pid, "kill");
        });
    };
