...
TopPanelApplication::TopPanelApplication()
    : LayerApplication("top_panel", 2) // 2 = ZWLR_LAYER_SHELL_V1_LAYER_TOP
{
    set_name("Top Panel");
    set_show_in_dock(false);
    
    DesktopEntry::add_search_path("/home/horacio/Desarrollo/austral-os/horizon/examples/config/apps/");
...
void TopPanelApplication::setup_ipc()
{
    ...
    m_ipc_client->subscribe("{\"type\": \"subscribe\"}", [this](const std::string& msg) {
        try {
